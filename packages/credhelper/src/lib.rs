...
fn get_or_create_entitlement_key<S: CredentialStore>(
    ...
    store.get(key)?.ok_or_else(|| {
        StoreError::Internal("entitlement key disappeared after its write".to_owned()).into()
    })
}
...
mod tests {
    ...
    fn existing_entitlement_key_always_wins_without_a_write() {
        ...
        store
            .values
            .borrow_mut()
            .insert(key.canonical_name(), b"ed25519:existing".to_vec());
        assert_eq!(
    ...
    fn entitlement_creation_returns_the_post_write_canonical_value() {
        let store = MemoryStore::default();
        *store.canonical_after_put.borrow_mut() = Some(b"ed25519:concurrent-winner".to_vec());
        let key = StoreKey::entitlement_key();
        assert_eq!(
            get_or_create_entitlement_key(&store, &key).unwrap(),
            b"ed25519:concurrent-winner"
        );
