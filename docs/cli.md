...
`build`, `configure`, `deploy`, `deployment`, `destroy`, `dev`, `down`, `env`, `init`, `link`, `logs`, `manifest`, `open`, `shell`, `stack`, `test`, `unlink`, and `up`.
...
- `appliance install` uses the same deploy engine but defaults to the local VM cluster. Pass `--cluster <name>` to install to another registered cluster (`--profile <name>` remains accepted for compatibility).
