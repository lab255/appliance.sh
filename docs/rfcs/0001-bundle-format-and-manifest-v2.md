...
| `publisher`                    | object                            | signed only         | omitted   | Optional on unsigned bundles; absence or unverifiable identity displays Unknown Publisher.    | none                                     |
| `publisher.name`               | string, 1–100                     | with `publisher`    | —         | Display name; never an authorization identity.                                                | none                                     |
| `publisher.keyId`              | string                            | with `publisher`    | —         | Stable Ed25519 public-key fingerprint used for trust and blacklist lookup.                    | none                                     |
...
| `services`                     | map service-name → service        | compound            | —         | 1–16 direct entries; maximum 16 services across the whole tree.                               | graph expansion follow-up                |
| `services.*.type`              | `container \| binary \| compound` | service             | —         | Nested service omits manifest, bundle, and publisher identity fields.                         | runner selection                         |
...
The omitted `web.isolation` resolves to `shared`. This RFC does not define startup order, health, restart, DNS,
cross-VM routing, or control composition. Those belong to the compound-graph spike; it must consume this minimal tree
without expanding the manifest shape here.
...
3. A signature requires `publisher.name` and `publisher.keyId`. A signature without the block is invalid.
4. `publisher.name` is display text. Trust, blacklist, and key lookup use `publisher.keyId` only.
5. Verification resolves the Ed25519 public key by key ID from the identity task's trust source, then verifies the
...
6. A publisher block without a signature remains Unknown Publisher and grants no trust.
...
bundle digest and publisher key ID; this RFC adds no hosting protocol.
...
9. Port, mount, service, and target names are duplicated; numeric limits are exceeded; references from `ui` do not
   resolve; a web UI points to a non-TCP or non-host port.
...
12. A compound has no services, exceeds 16 services across the tree, exceeds two service-containment levels, or puts
    root identity/signature fields inside a service.
13. `signature.sig` is malformed, is present without `publisher`, or fails verification with a resolved public key.
...
1. Add `packages/sdk/src/models/appliance-v2.ts` with strict schemas and inferred types for:
   ...
   - recursive `applianceV2ServiceInput` using `z.lazy` plus container, binary, and compound root schemas;
...
4. Add `packages/sdk/src/models/appliance-v2.spec.ts` covering every example, strict unknown-key rejection, each
   discriminant, Linux-target requirements, recursive depth/service count, cross-field references, SPDX/SemVer,
   ranges, path rules, native opt-in, and signature/publisher coupling.
...
4. **How is “two nesting levels” counted?** Default: the root is depth zero, its services are depth one, their services
   are depth two, and depth-three services are rejected; the 16-service cap counts all depths.
