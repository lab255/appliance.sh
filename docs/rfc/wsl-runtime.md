...
unions app grants in the VM-wide legacy policy; 3b confines that union to
credentialless legacy/dev requests and uses per-app selection for Runtime
tasks. Runtime policies are already normalized to default-deny
...
state, so the credential is absent from CLI arguments and ordinary app launch
...
but malformed, unknown, stale, or revoked credential receives 407 and never
falls back to the credentialless VM-wide policy.
